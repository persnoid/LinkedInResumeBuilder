...
function App() {
  ...
  useEffect(() => {
    const initializeData = async () => {
      // Try to load user's primary resume data from cloud
      try {
  ...
  const saveDraft = (name: string) => {
    ...
    const performSave = async () => {
      ...
      try {
        ...
        if (!user || authError) {
          throw new Error('You must be signed in to save drafts. Please sign in and try again.');
        }
  ...
  const autoSaveDraft = async (step: number) => {
    if (currentDraftId && resumeData) {
      try {
        ...
        if (!user || authError) {
          console.warn('Cannot auto-save: user not authenticated');
          return;
        }
